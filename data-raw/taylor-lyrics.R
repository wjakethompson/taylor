...
base_info <- lyrics %>%
  ...
  mutate(
    ...
    track_name = str_replace_all(track_name, "tis the damn", "'tis the damn"),
    track_name = str_replace_all(
      track_name,
      "no body no crime",
      "no body, no crime"
  ...
  mutate(
    ...
    track_name = str_replace_all(track_name, "Guilty As Sin", "Guilty As Sin?"),
    track_name = str_replace_all(
      track_name,
  ...
  mutate(
    ...
    track_name = case_when(track_name == "Us" ~ "us.", .default = track_name),
    track_name = str_replace_all(
      track_name,
...
single_uri <- tribble(
  ...
  "",
  "Bad Blood (Remix)",
  "6xsEAm6w9oMQYYg3jkEkMT",
  "Beautiful Ghosts",
  "2evEoQAhIMaa9PfjTT5skG",
  "Carolina",
  ...
  "Eyes Open",
  "6KEemo78n0RnCQWKkeOdXz",
  "Eyes Open (Taylor's Version)",
  "0gPmM1WtVRg3uENW3o8bfx",
  "I Don't Wanna Live Forever",
  "2y5aJvzXhHPA94U5GFAcXe",
  "If This Was A Movie (Taylor's Version)",
  "0q6vsjvnCIWrtwRckCV7y0",
  "Lover (Remix)",
  ...
  "September",
  "5eGX87IiKsGuzS3iw4CfCX",
  "Sweeter Than Fiction",
  "0RFCHlNuTeUHIB36VuVbOL",
  "Today Was A Fairytale",
  "4pFvEWbjBpPUdYRQly0THs"
)
...
feature_uri <- tribble(
  ...
  ~track_uri,
  "Babe",
  "7FFfYM4JE1vj5n4rhHxg8q",
  "Birch",
  "7wo2UNeQBowm28hfAJsEMz",
  "Both Of Us",
  "3r9bgSJlJz2zlevcBRYXko",
  "Gasoline (Remix)",
  "645Exr2lJIO45Guht3qyIa",
  "Half Of My Heart",
  "",
  "Highway Don't Care",
  "4wFUdSCer8bdQsrp1M90sa",
  "Renegade",
  "1aU1wpYBSpP0M6IiihY5Ue",
  "The Alcott",
  ...
  "1MaqkdFNIKPdpQGDzme5ss",
  "us.",
  "0hhzNPE68LWLfgZwdpxVdR"
)
...
writer_uri <- tribble(
  ...
  "16jvUOIQ2P54P0bNN4rAdv",
  "Better Man",
  "23TxRN09aR1RB0G0tFoT0b",
  "deja vu",
  "6HU7h9RYOaPRFeh0R3UeAr",
  "This Is What You Came For",
  "0azC730Exh71aQlOt9Zj3y",
  "TMZ",
  "2Y9YdGPN7AtVfpTvXfRbc6",
  "You'll Always Find Your Way Back Home",
  "12wSL3tGk3MtbDEhfG7xy3"
)
...
spotify_join <- spotify %>%
  ...
  mutate(
    track_name = case_when(
      str_detect(album_name, "folklore|evermore") ~ track_name,
      TRUE ~ str_to_title(track_name)
    ),
    track_name = str_replace_all(track_name, "’", "'")
  ) %>%
...
taylor_all_songs <- base_info %>%
  ...
  mutate(
    ...
    artist = case_when(
      track_name == "Half Of My Heart" ~ "John Mayer",
      track_name == "Three Sad Virgins" ~ "Please Don't Destroy",
      is.na(artist) ~ "Taylor Swift",
      .default = artist
    ),
    featuring = case_when(
      track_name == "Half Of My Heart" ~ "Taylor Swift",
...
metacritic <- tribble(
  ...
  mutate(
    ratings = map(metacritic_name, function(.x) {
      ...
      tibble(
        metacritic_score = as.integer(critic),
