...
test_that("album scale works", {
  ...
  # reverse levels
  rlvl_studio <- studio
  rlvl_studio$album_name <- factor(
    rlvl_studio$album_name,
    levels = rev(album_levels)
  )
  reverse_level <- ggplot(
    rlvl_studio,
    aes(x = metacritic_score, y = album_name)
  ) +
    geom_col(aes(fill = album_name)) +
  ...
  rand_studio <- studio
  new_levels <- c(
    seq(1, length(album_levels), by = 2),
    seq(2, length(album_levels), by = 2)
  )
  rand_studio$album_name <- factor(
    rand_studio$album_name,
